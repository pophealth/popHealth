...
<!-- Beta Skeleton Module for the Schematron 1.6 XML Schema Language.
...
<!-- 
    ...
    Contributors: Rick Jelliffe (original), Oliver Becker (architecture), 
	     ...
	     Francis Norton (generate-id-from-path), Robert Leftwich
...
This program implements schematron 1.6 except abstract schemas, which require a preprocess -->
...
<xsl:template match="sch:schema | schema">
	<axsl:stylesheet version="1.0">
		...
		<xsl:if test="count(sch:title/* | title/* )">
		 ...
		 <xsl:if test="not($phase = '#ALL')">
			  <xsl:if test="not(sch:phase[@id = $phase] | phase[@id = $phase])">
				  <xsl:message>Phase Error: no phase with name <xsl:value-of select="$phase"/> has been defined.</xsl:message>
		         </xsl:if>
              </xsl:if>
		...
		<xsl:apply-templates mode="do-keys" select="sch:key | pattern/rule/key "/>
...
<!--                <xsl:if test="//sch:rule[@id = current()/@pattern]"> -->
	...
	<xsl:template match="sch:key | key" mode="do-keys">
	     ...
	     <xsl:choose>
	     	<xsl:when test="parent::sch:rule | parent::rule">
	        <xsl:call-template name="IamEmpty"/>
	       <xsl:choose>
	       	<xsl:when test="@path">
